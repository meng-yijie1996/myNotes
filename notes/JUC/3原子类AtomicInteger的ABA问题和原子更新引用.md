...
<img src="../../pics/juc/aba/image-20200311212442057.png" alt="image-20200311212442057" style="zoom:50%;" />

假设现在有两个线程，分别是T1 和 T2，然后T1执行某个操作的时间为10秒，T2执行某个时间的操作是2秒，最开始AB两个线程，分别从主内存中获取A值，但是因为B的执行速度更快，他先把A的值改成B，然后再修改成A，然后执行完毕，T1线程在10秒后，执行完毕，判断内存中的值为A，并且和自己预期的值一样，它就认为没有人更改了主内存中的值，就快乐的修改成B，但是实际上 可能**中间**经历了 ABCDEFA 这个变换，也就是中间的值经历了狸猫换太子。

ABA问题：在进行获取主内存值的时候，该内存值在我们写入主内存的时候，已经被修改了N次，但是最终又改成原来的值了。

### CAS导致ABA问题

CAS算法实现了一个重要的前提，需要取出内存中某时刻的数据，并在当下时刻比较并替换，那么这个**时间差**的数据变化是无法感知的。
...
作用：将一个java类，用**原子引用类**包装起来，那么这个类就具备了原子性。

目的：解决使用普通的对象引用，在多线程情况下进行对象的更新可能会导致不一致性的问题。如

- 一个对象的初始状态为 `name=Tom, age = 18`。在 线程1 中将 `name` 修改为 `Tom1`，`age + 1`。在 线程2 中将 `name` 修改为 `Tom2`，`age + 2`。
- 实际是：`name: Tom1, age: 21`、`name: Tom1, age: 21`
...
public class ABADemo {
    ...
    public static void main(String[] args) {
        ...
        new Thread(() -> {
            // 睡眠一秒，保证t1线程完成了ABA操作：即使t2先启动，它也会先停1秒，让t1执行。
...
结果：true  2019。能够成功的修改，这就是ABA问题。

### 解决ABA问题

新增一种机制：修改版本号，类似于时间戳的概念
...
![image-20200312200434776](../../pics/juc/aba/image-20200312200434776.png)
...
![image-20200429085540554](../../pics/juc/aba/image-20200429085540554.png)
...
![img](../../pics/juc/aba/image-20200429085141487.png)
...
最后，如果你要从LongAdder中获取当前累加的总值，就会把base值和所有Cell分段数值**加起来**返回给你。

![image-20200429085957778](../../pics/juc/aba/image-20200429085957778.png)
...
![image-20200429090249633](https://gitee.com/moxi159753/LearningNotes/raw/master/%E6%A0%A1%E6%8B%9B%E9%9D%A2%E8%AF%95/JUC/3_%E8%B0%88%E8%B0%88%E5%8E%9F%E5%AD%90%E7%B1%BB%E7%9A%84ABA%E9%97%AE%E9%A2%98/6_%E5%8E%9F%E5%AD%90%E7%B1%BBAtomicInteger%E7%9A%84ABA%E9%97%AE%E9%A2%98/images/image-20200429090249633.png)
...
[AtomicLong与LongAdder（CAS机制的优化）](https://gitee.com/link?target=https%3A%2F%2Fblog.csdn.net%2Feluanshi12%2Farticle%2Fdetails%2F84871879)

[大白话聊聊Java并发面试问题之Java 8如何优化CAS性能？](https://gitee.com/link?target=https%3A%2F%2Fmp.weixin.qq.com%2Fs%2FKFsqsCVgyiiTDXMR-Hu1-Q)

[https://blog.csdn.net/wolf_love666/article/details/87693771](https://gitee.com/link?target=https%3A%2F%2Fblog.csdn.net%2Fwolf_love666%2Farticle%2Fdetails%2F87693771)